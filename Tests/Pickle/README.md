...
dependencies; `wsl-deps.base.map` stages the minimal declared set; and
`wsl-deps.perfumes.map`, `wsl-deps.anima.map`, and `wsl-deps.social.map` each activate one
integration, its non-core dependencies, and the local RimScent Extended checkout. The package
identities were read from the locally installed Workshop copies on 2026-09-26. Every submission
must name its relevant map with `-DepMap`; no default map is assumed.
